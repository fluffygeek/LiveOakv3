...
/** Test double — never used by production code (mirrors
 * functions/src/jobRecords/inMemoryJobRecordRepository.ts's role in the Firebase-side
 * tests; kept minimal here since this file only exercises the handler's transport/auth
 * wiring, not real Duplicate matching — see jobRecordService.test.ts for that). */
class InMemoryJobRecordRepository implements JobRecordRepository {
  ...
  // Mirrors the real PostgresJobRecordRepository's #21 behavior (see
  // supabase/functions/_shared/postgresJobRecordRepository.ts): always zero matches, plain
  // insert of the resulting record.
...
function request(body: unknown): Request {
  return new Request("http://localhost/createJobRecord", {
    method: "POST",
...
Deno.test("createJobRecord - a Technician can submit a Job Record, and it's stored", async () => {
  ...
  assertEquals(body.data.jobId, "DISPATCH-123");
  assertEquals(body.data.technicianEmail, "tech@example.com");
  assertExists(body.data.recordId);

  const stored = await jobRecordRepo.getById(body.data.recordId);
  assertEquals(stored?.jobId, "DISPATCH-123");
});

Deno.test("createJobRecord - a non-Technician caller is rejected", async () => {
  const userRepo = new InMemoryUserRepository();
  userRepo.seed({
...
Deno.test("createJobRecord - invalid input (missing jobId) is a 400", async () => {
  const userRepo = new InMemoryUserRepository();
  userRepo.seed(technicianRecord("tech@example.com"));
  const handler = createHandler(
    userRepo,
  ...
  const { jobId: _jobId, ...withoutJobId } = validInput();
  const response = await handler(request(withoutJobId));
...
Deno.test("createJobRecord - an unauthenticated caller gets 401, never reaching the repository", async () => {
