...
function request(body: unknown): Request {
  return new Request("http://localhost/inviteUser", {
    method: "POST",
...
Deno.test("inviteUser - an Application Administrator can invite a new user", async () => {
  const repo = new InMemoryUserRepository();
  repo.seed(adminRecord("admin@example.com"));
  const handler = createHandler(repo, fakeVerifyCaller("admin@example.com"));

  const response = await handler(
    request({ email: "tech@example.com", roles: ["technician"] }),
  );

  assertEquals(response.status, 200);
  const body = await response.json();
  assertObjectMatch(body.data, {
  ...
  assertEquals(stored?.roles, ["technician"]);
});

Deno.test("inviteUser - a non-administrator caller is rejected", async () => {
  const repo = new InMemoryUserRepository();
  repo.seed({
    email: "tech@example.com",
    roles: ["technician"],
  ...
  const response = await handler(
    request({ email: "newperson@example.com", roles: ["technician"] }),
  );
...
Deno.test("inviteUser - re-inviting the sole active administrator without the admin role trips LastAdministratorError", async () => {
  const repo = new InMemoryUserRepository();
  repo.seed(adminRecord("admin@example.com"));
  const handler = createHandler(repo, fakeVerifyCaller("admin@example.com"));
...
Deno.test("inviteUser - an invalid role in the payload is a 400", async () => {
  const repo = new InMemoryUserRepository();
  repo.seed(adminRecord("admin@example.com"));
  const handler = createHandler(repo, fakeVerifyCaller("admin@example.com"));

  const response = await handler(
    request({ email: "tech@example.com", roles: ["not-a-real-role"] }),
  );
