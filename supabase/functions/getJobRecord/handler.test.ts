 ...
 * getJobRecord's transport/auth wiring, not Duplicate matching. */
...
function request(body: unknown): Request {
  return new Request("http://localhost/getJobRecord", {
    method: "POST",
...
Deno.test("getJobRecord - a Payroll/Application Administrator can fetch a Job Record by id", async () => {
  ...
  assertEquals(body.data.recordId, "record-1");
});

Deno.test("getJobRecord - a Technician caller (not an admin) is rejected", async () => {
  const userRepo = new InMemoryUserRepository();
  userRepo.seed({
...
Deno.test("getJobRecord - an unknown recordId is a 404", async () => {
  const userRepo = new InMemoryUserRepository();
  userRepo.seed(adminRecord("admin@example.com"));
...
Deno.test("getJobRecord - an unauthenticated caller gets 401, never reaching the repository", async () => {
