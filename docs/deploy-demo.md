...
Every admin-only Edge Function (`inviteUser`, `updateUserRoles`, etc.) calls
`resolveAccess`/`requireApplicationAdministrator`
...
**Verified live** in this environment (Docker + `supabase@2.114.0`, `npx supabase start`):
this returns the inserted row with `invited_at`/`updated_at`
...
Once that row exists and Google sign-in is configured (see
...
sequenceDiagram
    ...
    You->>Web: sign in — blocked locally today by the missing Google OAuth client
```
...
simulator. As with the web app, signing in is blocked locally today by the missing Google
OAuth client (see [Known gaps](#known-gaps-worth-disclosing-before-the-demo)).
